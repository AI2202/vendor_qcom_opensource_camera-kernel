...
#if defined ASUS_AI2202_PROJECT
  pr_err("[OIS] for AI2202, ois is controlled by sensor");
	return 0;
#endif
...
int32_t IIC_DataSend(unsigned short addr, uint32_t size, unsigned char* SendData) {
	...
	if (size<255) {
		...
		pr_err("[OIS] DataSend, size=%d > 255, skip", size);
	}
#endif
...
void IIC_WaitSendByte(unsigned short addr, unsigned char SendData) {	
...
void FlashWriteResultCheck(void) {
	unsigned char SendData;
	unsigned char RcvData;
	...
	Wait(190msec); /* Wait for Flash ROM Write + 20msec is margin in low frequency clock */
	do
	{
...
if (checkHWBoard()==AI2202_EVB) {
	...
	IIC_WaitSendByte(0x0003, 0x01);
	SendData=1;
	IIC_DataSend(0x0003, 1, &SendData);
	Wait (190); /* Wait for Flash ROM Write + 20msec is margin in low frequency clock */
	IIC_DataRead(0x03, 1, &RcvData);
	if( RcvData != 0 )
	{
...
#define S4W_FULL_Size 28672
	...
	unsigned long RcvDataLong;
	unsigned long NewFWRev;
	int WriteCnt;
	unsigned short checkSum;
...
//	NewFWRev = Read target Binary File address 0x6FF4-0x6FF7; (*1)
	NewFWRev = *(long*)&Firmware_Data_Buffer[0x6FF4];

	/* Check RUMBA device */
	IIC_DataRead(0xFC, 4, SendData);
	RcvDataLong = *(long*)&SendData[0];
	...
	if ((int)RcvDataLong>=(int)NewFWRev) {
		pr_err("[OIS] FW  binary file version=%d, module firmware version=%d, module firmware newer skip", NewFWRev, RcvDataLong);
		return;
	}

	pr_err("[OIS] FW binary file version=%d, firmware version=%d, Start to update firmware, diff=%d", NewFWRev, RcvDataLong, RcvDataLong-NewFWRev);
#if 1
...
for (iRetry=0;iRetry<I2C_BLOCK_RETRY;iRetry++){
	...
	/* F/W Update Error check */
	{
		...
		/* F/W Update Error check */
		{
			IIC_DataRead(0xFC, 4, (unsigned char*)&RcvDataLong); /* Read revision of writing F/W */
			if((int)RcvDataLong==(int)NewFWRev)
			/* Check Writing F/W Rev. */
			{
			/* F/W Update Success Process */
				pr_err("[OIS] FW update OK, new version=%d", RcvDataLong);
				FlashWriteResultCheck();			
				break;
			} 
			else {
			/* F/W Update Error Process */
				pr_err("[OIS] FW update Fail, version dismatch, current version=%d, expect to %d", RcvDataLong, NewFWRev);
			}
		} else {
			/* F/W Update Error Process */
